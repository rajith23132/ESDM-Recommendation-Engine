...
# Update the tab definition at the top of the code
...
with tab5:
    ...
    <h1 class="centered">👁️ Visual Component Search</h1>
    <p class="centered" style="color: #666;">Upload an image to find similar components</p>
    """, unsafe_allow_html=True)
    
    # Image upload section with more specific guidance
    uploaded_file = st.file_uploader(
        "Upload ESDM Component (IC, resistor, capacitor, etc.)",
        type=["jpg", "jpeg", "png"],
        help="Example: Clear photos of chips, transistors, or other electronic parts with visible markings"
    )
    
    if uploaded_file is not None:
        # Display the uploaded image with a warning
        st.image(uploaded_file, use_container_width=False,width=500,output_format="auto")
                   
        # Process the image when user clicks the button
        if st.button("🔍 Component Search", type="primary"):
            with st.spinner("Analyzing image for electronic components..."):
                try:
                    # Prepare the image for Gemini
                    image_bytes = uploaded_file.getvalue()
                    
                    # Gemini Vision API call
                    api_key = "AIzaSyBgNyOek4g4RhmY_-JG_lxG1n5kA-yKUE8"
                    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={api_key}"
                    headers = {"Content-Type": "application/json"}
                    ...
                    # More specific prompt focusing on electronic components
                    prompt = """
                    STRICTLY analyze this image ONLY for electronic components. 
                    If this is NOT an electronic component, return:
                    {
                        "is_electronic": false,
                        "content_type": "description of actual content",
                        "message": "This does not appear to be an electronic component"
                    }
                    
                    If it IS an electronic component, return:
                    {
                        "is_electronic": true,
                        "component_type": "specific type (e.g., microcontroller, capacitor)",
                        "key_features": "pin count, markings, package type",
                        "likely_specs": "voltage range, current rating",
                        "common_applications": "typical uses",
                    ...
                    raw_text = result["candidates"][0]["content"]["parts"][0]["text"]
                    cleaned_text = re.sub(r"^```json|```$", "", raw_text.strip(), flags=re.MULTILINE).strip()
                    vision_analysis = json.loads(cleaned_text)
                    
                    # Display the analysis
                    st.markdown("### 🧠 Image Analysis Results")
                    st.json(vision_analysis)
                    
                    # Check if image contains electronic components
                    if not vision_analysis.get("is_electronic", True):
                        st.error("❌ No Electronic Component Detected")
                        st.markdown(f"""
                        ### This appears to be: {vision_analysis.get('content_type', 'unknown content')}
                        
                        **Message**: {vision_analysis.get('message', 'The image does not contain recognizable electronic components')}
                        
                        """)
                        st.stop()
                    
                    # Validate electronic component analysis
                    required_fields = ["component_type", "key_features"]
                    for field in required_fields:
                        if field not in vision_analysis:
                            raise ValueError(f"Missing required field in response: {field}")
                    
                    # Use the analysis to find similar components
                    search_terms = [
                        vision_analysis['component_type'],
                        *vision_analysis.get('possible_part_numbers', []),
                        vision_analysis['key_features']
                    ]
                    search_query = " ".join(filter(None, search_terms))
                    
                    st.markdown(f"### 🔍 Searching for: `{search_query}`")
                    
                    extracted = extract_keywords_with_gemini(search_query)
                    results = retrieve_similar_products(extracted, top_k=8)
                    
                    if not results:
                        st.error("🔍 No Matching Components Found")
                        st.markdown(f"""
                        ### Couldn't find matches for: {vision_analysis['component_type']}
                        
                        Try:
                        1. A different image of the same component
                        2. Manual search using the component type
                        3. Checking the spelling of any part numbers
                        """)
                    else:
                        st.session_state.recommendations = results
                        st.markdown("### 🔍 Matching Components")
                        
                        # Filter out irrelevant matches (e.g., buses)
                        valid_components = [
                            p for p in results 
                            if (vision_analysis['component_type'].lower() in p['product_type'].lower() or
                                any(pn.lower() in p['product_name'].lower() 
                                    for pn in vision_analysis.get('possible_part_numbers', [])))
                        ]
                        
                        if not valid_components:
                            st.error("⚠️ No relevant components found")
                            st.info(f"Found components but none match: {vision_analysis['component_type']}")
                        else:
                            cols = st.columns(2)
                            for i, item in enumerate(valid_components[:6]):
                                with cols[i % 2]:
                                    with st.expander(f"🏅 {item['product_name']}", expanded=True):
                                        st.markdown(f"**Type**: {item['product_type']}")
                                        st.markdown(f"**Manufacturer**: {item['company_name']}")
                                        st.markdown(f"**Price**: ₹{item.get('price', 'N/A')}")
                                        
                                        if item.get('pin_count'):
                                            st.markdown(f"**Pins**: {item['pin_count']}")
                                        if item.get('voltage_range'):
                                            st.markdown(f"**Voltage**: {item['voltage_range']}V")
                                        if item.get('power_consumption'):
                                            st.markdown(f"**Power**: {item['power_consumption']}mW")
                                        
                                        col1, col2 = st.columns(2)
                                        with col1:
                                            st.metric("♻️ Green", item.get('green_index', 'N/A'))
                                        with col2:
                                            st.metric("💡 Innovation", item.get('innovation_score', 'N/A'))
                                        
                                        if item.get('product_url'):
                                            st.markdown(f"[Product Page]({item['product_url']})")
                        
                except Exception as e:
                    st.error(f"Not an ESDM Component!!!!!!!!!")
                    st.info("Please try a clearer image or different component")
